...
-------------------------------------------------------------------------------]]
local Type, Version = "GargulIncomingPlusTwoDataDialog", 1
...
local function constructor()
    local Frame = GL.Interface:get(GL.Interface.Dialogs.IncomingPlusTwoDataDialog, "Window");
    if (Frame) then
        Frame:Hide();
    end

    local frameName = "GargulIncomingPlusTwoDataDialog";
    ...
    GL.Interface:set(GL.Interface.Dialogs.IncomingPlusTwoDataDialog, "Window", Frame);
    ...
    if (sender) then
        ...
        GL.Interface:set(GL.Interface.Dialogs.IncomingPlusTwoDataDialog, "TrustSender", TrustSenderCheckBox);
        Widget.TrustSenderCheckBox = TrustSenderCheckBox;

        TrustSenderCheckBox:SetCallback("OnValueChanged", function (widget)
            if (widget:GetValue()) then
                GL.PlusTwos:markPlayerAsTrusted(sender);
            else
                GL.PlusTwos:removePlayerFromTrusted(sender);
            end
        end);
    ...
    GL.Interface:set(GL.Interface.Dialogs.IncomingPlusTwoDataDialog, "BlockSharedData", BlockSharedDataCheckBox);
    Widget.BlockSharedDataCheckBox = BlockSharedDataCheckBox;

    BlockSharedDataCheckBox:SetCallback("OnValueChanged", function (widget)
        if (widget:GetValue()) then
            GL.Settings:set("PlusTwos.blockShareData", true);
        else
            GL.Settings:set("PlusTwos.blockShareData", false);
        end
    end);
...
GL:debug("Interface/IncomingPlusTwoDataDialogWidget.lua");